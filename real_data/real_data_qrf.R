# rm(list = ls())
# ==============================================================================
# Script: real_data_qrf.R
# Description: Conducts real-data experiments using Quantile Regression Forests
#              (QRF) as a baseline comparison for the BAND framework.
...
# Configuration matching real_data_band.R
...
for (target_vars in coordinate_combinations) {
  ...
  for (lag in c(1, 3, 6)) {
    ...
    for (period_name in names(CUTOFF_SETS)) {
      ...
      # Fit one QRF per response variable in the target set
      qrf_models <- list()
      for (col_name in target_vars) {
        col_ind <- which(COORDINATE_SET == col_name)
        qrf_models[[col_name]] <- quantregForest(
          x = df_train[, c("X1", "X2", "X3")],
          y = df_train[, paste0("Y", col_ind)],
          keep.inbag = TRUE
        )
      }
      ...
      # Calculate Joint Coverage using Bonferroni correction
      for (i in seq_along(ALPHA_LEVELS)) {
        alpha <- ALPHA_LEVELS[i]
        k     <- length(target_vars)
        
        # Bonferroni adjustment for joint interval
        beta      <- 1 - (1 - alpha) / k
        tau_lower <- (1 - beta) / 2
        ...
        for (row in 1:nrow(df_test)) {
          ...
          for (col_name in target_vars) {
            ...
            # Predict quantiles
            preds <- predict(qrf_models[[col_name]], 
                             newdata = df_test[row, c("X1", "X2", "X3"), drop = FALSE], 
                             what = c(tau_lower, tau_upper))
            
            if (!(y_actual >= preds[1] && y_actual <= preds[2])) {
              in_interval <- FALSE
              break
            }
          }
      ...
      cat(sprintf("[%s] QRF Var: %s | Lag: %d | Status: Complete\n", 
...
# MAKE PLOTS
### ------------------------
file_path <- here::here('real_data', 'real_data_results', 'qrF.rds')
...
for (coordinate_set in names(results)) {
  for (period_name in names(results[[coordinate_set]])) {
    ...
    img_name <- paste0(
      ...
      "_period_", period_name, "_QRF_.png"
    )
    img_path <- here::here("real_data", 'qrf', img_name)
